...
RESULTS=os.path.join(config["resultsdir"],str(config["expiriment"]+"/"))
SAMPLES = expand(config["samples"])
SOURCE = config["source"]
ASSAY = config["expiriment"]
...
rule all:
	input:
		...
		# Mapping stats
		RESULTS+"analysis/merge/All.Stats.tsv",
		...
		expand(RESULTS+"data/stats/{sample}.map.stats.txt",sample=SAMPLES),
...
rule minimap2splice:
	...
	shell:
		"minimap2 -t {threads} -ax splice {input.index} {input.reads} | samtools sort -@ 20 | samtools view -hbS -@ 20 > {output}"
...
rule minimap2full:
	...
	output:
		bam=temp(RESULTS+"data/bam/{sample}.transcripts.bam"),
		sbam=RESULTS+"data/bam/{sample}.transcripts.sorted.bam"
	threads: config["threads"]
	shell:"""
	minimap2 -t {threads} -ax map-ont -p 1.0 -N 100 {input.index} {input.reads} | samtools view -Sb > {output.bam};
...
rule makeTranscriptome:
	input:
		gff=RESULTS+"data/transcriptome/"+ASSAY+".ref.trps.gtf",
		reference = config["ref_genome"]
	output:
		trp_fa = RESULTS+"data/transcriptome/"+ASSAY+".ref.trps.fa"
	shell:
		"gffread -w {output.trp_fa} -g {input.reference} {input.gff} -F"
...
rule mapStats:
	...
	output:
		samtoolsOut=RESULTS+"data/stats/{sample}.map.stats.txt",
		readDist=temp(RESULTS+"data/stats/{sample}.read-stats.raw.txt")
	shell:"""
		samtools flagstat {input.bam} > {output.samtoolsOut};
...
rule makeTPM:
	...
	shell:
		"stringtie {input.alignment} -G {input.annotation} -L -A {output.abundance} > {output.gtf}"
